...
public class LogUtils {
	...
	public static void LogKaiMenFile(String name, String message) {

		LogKMToSDFile(name, message);
	...
	//
	// /**
	// * 把log日志输出到data/data/包名文件夹/files/下
	// * <br> 除非特殊情况,一般不建议输出到这个位置,建议输出到SD卡中
	// * @param message 要写入的信息
	// */
	// public static void LogToDataFile(String message) {
	// String path =
	// InsideApplication.getApplication().getFilesDir().getAbsolutePath()
	// + File.separator + LOG_FILE_NAME;
	// logToFile(path, getCurrentTime()+" "+ message);
	// }
	//
	// public static void deleteDataLogFile() {
	// String path =
	// InsideApplication.getApplication().getFilesDir().getAbsolutePath()
	// + File.separator + LOG_FILE_NAME;
	// deleteLogFile(path);
	...
	//
	// /**
	// * cache目录和files目录都是缓存目录,区别在于,cache目录下的文件,如果系统缺空间了,会自动帮我们删,files只能我们手动删
	// * @param message 要写入的信息
	// */
	// public static void LogToDataCacheFile(String message) {
	// String path =
	// InsideApplication.getApplication().getCacheDir().getAbsolutePath()
	// + File.separator + LOG_FILE_NAME;
	// logToFile(path, getCurrentTime()+" "+ message);
	// }
	...
	public static void LogToSDFile(String message) {
		...
		String path = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + LOG_FILE_NAME;
		logToFile(path, getCurrentTime() + "  " + getJavaFileName(4) + "--" + message);
	}

	public static void LogKMToSDFile(String name, String message) {
		// 判断是否有SD卡
		if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
			return;
		}
		String path = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + name + ".log";
		logToFile(path, getCurrentTime() + "  " + getJavaFileName(4) + "--" + message);
	}
	...
	// ----------------帮助方法----------------------
	private static String LOG_FILE_NAME = "LogUtils.log";
